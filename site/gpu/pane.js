...
export function setupPane() {
  ...
  const heerich = pane.addFolder({
    title: "Context free system",
    expanded: false,
  });
  heerich.addBinding(PARAMS, "maxDepth", {
    min: 2,
    max: 30,
    step: 1,
    label: "max depth",
  });
  heerich.addBinding(PARAMS, "rootSize", {
    min: 2,
    max: 20,
    step: 1,
    label: "root size",
  });
  heerich.addBinding(PARAMS, "trunkChance", {
    min: 0,
    max: 1,
    step: 0.01,
    label: "trunk prob",
  });
  heerich.addBinding(PARAMS, "sideChance", {
    min: 0,
    max: 0.8,
    step: 0.01,
    label: "side prob",
  });
  heerich.addBinding(PARAMS, "scaleDecay", {
    min: 0.3,
    max: 0.95,
    step: 0.01,
    label: "scale decay",
  });
  heerich.addBinding(PARAMS, "spread", {
    min: 0.9,
    max: 2.0,
    step: 0.01,
    label: "spread",
  });
  heerich.addBinding(PARAMS, "upBias", {
    min: -1,
    max: 1,
    step: 0.05,
    label: "up bias",
  });
  heerich.addBinding(PARAMS, "hueStart", {
    min: 0,
    max: 360,
    step: 1,
    label: "hue start",
  });
  heerich.addBinding(PARAMS, "hueRange", {
    min: 0,
    max: 360,
    step: 1,
    label: "hue range",
  });
  heerich.addBinding(PARAMS, "sat", {
    min: 0,
    max: 1,
    step: 0.01,
    label: "Saturation",
  ...
  camFolder
    .addBinding(PARAMS, "dofFocalRange", {
      min: 1,
      max: 200,
      step: 1,
      label: "Focal range",
    ...
    .on("change", ({ value }) => {
      focalRangeUniform.value = value;
    });
  camFolder
    .addBinding(PARAMS, "dofBokeh", {
      min: 0,
      max: 10,
      step: 0.1,
      label: "Bokeh scale",
  ...
  ptFolder
    .addBinding(PARAMS, "bloomStrength", {
      min: 0,
      max: 3,
      step: 0.05,
  ...
  emFolder
    .addBinding(PARAMS, "emissiveChance", {
      min: 0,
      max: 0.5,
      step: 0.01,
      label: "Chance",
